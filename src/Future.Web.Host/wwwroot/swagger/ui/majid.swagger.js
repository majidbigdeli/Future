...
(function () {
    ...
    majid.swagger = majid.swagger || {};

    majid.swagger.addAuthToken = function () {
        var authToken = majid.auth.getToken();
        if (!authToken) {
            return false;
        }

        var cookieAuth = new SwaggerClient.ApiKeyAuthorization(majid.auth.tokenHeaderName, 'Bearer ' + authToken, 'header');
        swaggerUi.api.clientAuthorizations.add('bearerAuth', cookieAuth);
        return true;
    }

    majid.swagger.addCsrfToken = function () {
        var csrfToken = majid.security.antiForgery.getToken();
        if (!csrfToken) {
            return false;
        }
        var csrfCookieAuth = new SwaggerClient.ApiKeyAuthorization(majid.security.antiForgery.tokenHeaderName, csrfToken, 'header');
        swaggerUi.api.clientAuthorizations.add(majid.security.antiForgery.tokenHeaderName, csrfCookieAuth);
        return true;
    }
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    ...
                    majid.auth.setToken(result.accessToken, expireDate);
                    callback();   
                } else {
                    alert('Login failed !');
                }
    ...
    majid.swagger.logout = function () {
        majid.auth.clearToken();
    }

    majid.swagger.closeAuthDialog = function () {
        if (document.getElementById('majid-auth-dialog')) {
            document.getElementsByClassName("swagger-ui")[1].removeChild(document.getElementById('majid-auth-dialog'));
        }
    }

    majid.swagger.openAuthDialog = function (loginCallback) {
        majid.swagger.closeAuthDialog();

        var majidAuthDialog = document.createElement('div');
        majidAuthDialog.className = 'dialog-ux';
        majidAuthDialog.id = 'majid-auth-dialog';

        document.getElementsByClassName("swagger-ui")[1].appendChild(majidAuthDialog);

        // -- backdrop-ux
        var backdropUx = document.createElement('div');
        backdropUx.className = 'backdrop-ux';
        majidAuthDialog.appendChild(backdropUx);

        // -- modal-ux
        var modalUx = document.createElement('div');
        modalUx.className = 'modal-ux';
        majidAuthDialog.appendChild(modalUx);
        ...
        modalUxContent.onkeydown = function (e) {
            if (e.keyCode === 13) {
                ...
                majid.swagger.login(loginCallback);
            }
        };
        ...
        closeButton.onclick = majid.swagger.closeAuthDialog;
